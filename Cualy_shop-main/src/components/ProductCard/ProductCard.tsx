...
const ProductCard = ({ id, nombre, categoria, precioNormal, precioOferta, imagenes, youtubeUrl, variantes = [], enOferta = false }: ProductCardProps) => {
  ...
  const getOptimizedImageUrl = (url: string) => {
    if (!url || typeof url !== 'string') return 'https://via.placeholder.com/400x300?text=Cualy+Shop';
    
    // Supabase image transformation
    if (url.includes('supabase.co/storage/v1/object/public/')) {
      // Usar parámetros de transformación de Supabase si están disponibles
      // Nota: Esto requiere que el bucket sea público y tenga transformaciones habilitadas
      // Por ahora, solo nos aseguramos de que la URL sea válida
      return url;
    }
    return url;
  };
  ...
  const handleAction = (e: React.MouseEvent, mode: 'cart' | 'buy') => {
    ...
    if (mode === 'buy' && variantes.length === 0) {
      ...
      const message = `Hola Cualy Shop, estoy interesado en comprar: *${nombre}*${priceText}\nLink: ${window.location.origin}/producto/${id}`;
      window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`, '_blank');
      return;
    }

    setQuickSelectMode(mode);
    // Removed automatic variant selection as per user request
    setSelectedVariants({});
  ...
  const executeAction = (mode: 'cart' | 'buy', selected: Record<string, string>, priceO: number, priceN: number) => {
    ...
    if (mode === 'buy') {
      ...
      const message = `Hola Cualy Shop, estoy interesado en comprar: *${finalName}* (x${quantity})${priceText}\nLink: ${window.location.origin}/producto/${id}`;
      window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`, '_blank');
            ...
            <div className="quickSelectHeader">
              <div className="quickPreviewWrapper">
                <img 
                  ...
                  onError={(e) => { (e.target as HTMLImageElement).src = 'https://via.placeholder.com/100x100?text=Cualy'; }}
                />
              </div>
